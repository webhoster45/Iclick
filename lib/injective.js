...
export async function broadcastEip712({ msgs, injectiveAddress, ethereumAddress, provider }) {
  ...
  const envChainId = Number.parseInt(process.env.NEXT_PUBLIC_EVM_CHAIN_ID || '', 10);
  const defaultEvmChainId = Number.isFinite(envChainId) ? envChainId : 1439;
  let ethereumChainId = defaultEvmChainId;
  let detectedChainId = null;
  try {
    const chainIdHex = await provider.request({ method: 'eth_chainId' });
    if (typeof chainIdHex === 'string') {
      const detected = parseInt(chainIdHex, 16);
      if (Number.isFinite(detected)) {
        detectedChainId = detected;
      }
    }
  } catch {
    // Use defaultEvmChainId when provider chain id cannot be detected
    ethereumChainId = defaultEvmChainId;
  }
  if (detectedChainId && detectedChainId !== defaultEvmChainId) {
    throw new Error(
      `Wallet network mismatch. Please switch your wallet to Injective EVM Testnet (chainId ${defaultEvmChainId}).`
    );
  }
  ethereumChainId = detectedChainId || defaultEvmChainId;

  const authApi = new ChainGrpcAuthApi(ENDPOINTS.grpc);
  const { baseAccount } = await authApi.fetchAccount(injectiveAddress);
  if (!baseAccount) {
    throw new Error(
      'Wallet not initialized on Injective. Fund the address with testnet INJ and complete one on-chain action to register the public key.'
    );
  }
  ...
  const recoveredPubKey = recoverTypedSignaturePubKey(eip712TypedData, signature);
  const pubKey = hexToBase64(recoveredPubKey);
...
/**
 * Build a default Injective subaccount id (0x + 32 bytes).
 * Format: 0x + 20-byte EVM address + 12-byte subaccount nonce (all zeros).
 */
export function buildSubaccountId(evmAddress) {
  if (!/^0x[0-9a-fA-F]{40}$/.test(evmAddress || '')) {
    throw new Error('Invalid EVM address for subaccount id');
  }
  return `0x${evmAddress.slice(2).toLowerCase()}${'0'.repeat(24)}`;
}
